...
class Attention(nn.Module):
    def __init__(self, num_classes=4, patch_size=100):
        super(Attention, self).__init__()
        self.num_classes = num_classes
        self.dimension = 50 * (((patch_size - 4) // 2 - 4) // 2)**2
        self.M = 500
        self.L = 128
        ...
        self.feature_extractor_part2 = nn.Sequential(
            nn.Linear(self.dimension, self.M),
            nn.ReLU()
        )
    ...
    def forward(self, x):
        ...
        H = H.contiguous().view(-1, self.dimension)
